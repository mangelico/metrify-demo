...
async def test_assemblyai_happy_path(assemblyai_fn, mock_m, with_ck):
    ctx = _http_mock(
        post_data={"id": "tx_abc123"},
        get_data={"status": "completed", "text": "Hello world transcription"},
    )
    with patch("httpx.AsyncClient", return_value=ctx):
        result = await assemblyai_fn("https://example.com/audio.mp3")
...
async def test_assemblyai_insufficient_balance(assemblyai_fn, mock_m, with_ck):
    mock_m._billing.check_balance.side_effect = InsufficientBalanceError("no funds")

    with pytest.raises(InsufficientBalanceError):
        await assemblyai_fn("https://example.com/audio.mp3")

    mock_m._billing.charge.assert_not_called()


async def test_assemblyai_gateway_error(assemblyai_fn, mock_m, with_ck):
...
async def test_assemblyai_transcription_error_no_charge(assemblyai_fn, mock_m, with_ck):
    ctx = _http_mock(
        post_data={"id": "tx_abc123"},
...
async def test_assemblyai_duration_too_long(assemblyai_fn, mock_m, with_ck):
    ctx = _http_mock(
        post_data={"id": "tx_abc123"},
        get_data={"status": "completed", "text": "...", "audio_duration": 301},
    )
    with patch("httpx.AsyncClient", return_value=ctx):
        result = await assemblyai_fn("https://example.com/long.mp3")
...
async def test_assemblyai_duration_missing_skips_check(assemblyai_fn, mock_m, with_ck):
    ctx = _http_mock(
        post_data={"id": "tx_abc123"},
