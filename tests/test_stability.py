...
async def test_stability_happy_path(stability_fn, mock_m, with_ck):
    ...
    with patch("httpx.AsyncClient") as mock_cls:
        ...
        result = await stability_fn("a sunset over the ocean")
...
async def test_stability_insufficient_balance(stability_fn, mock_m, with_ck):
    mock_m._billing.check_balance.side_effect = InsufficientBalanceError("no funds")

    with pytest.raises(InsufficientBalanceError):
        await stability_fn("a sunset")

    mock_m._billing.charge.assert_not_called()


async def test_stability_gateway_error(stability_fn, mock_m, with_ck):
...
async def test_stability_api_failure_no_charge(stability_fn, mock_m, with_ck):
    with patch("httpx.AsyncClient") as mock_cls:
        mock_cls.return_value.__aenter__ = AsyncMock(return_value=mock_cls.return_value)
        mock_cls.return_value.__aexit__ = AsyncMock(return_value=False)
        mock_cls.return_value.post = AsyncMock(side_effect=Exception("Stability 503"))
        result = await stability_fn("a sunset")
