...
async def test_anthropic_happy_path(anthropic_fn, mock_m):
    ...
    with patch("anthropic.AsyncAnthropic") as mock_cls:
        ...
        result = await anthropic_fn("Hello!", consumer_api_key="ck_test")
...
async def test_anthropic_insufficient_balance(anthropic_fn, mock_m):
    ...
    with pytest.raises(InsufficientBalanceError):
        await anthropic_fn("Hello!", consumer_api_key="ck_test")
...
async def test_anthropic_prompt_too_long(anthropic_fn, mock_m):
    long_prompt = "x" * 2001
    result = await anthropic_fn(long_prompt, consumer_api_key="ck_test")
    assert result.startswith("Error:")
...
async def test_anthropic_max_tokens_capped(anthropic_fn, mock_m):
    ...
    with patch("anthropic.AsyncAnthropic") as mock_cls:
        mock_cls.return_value.messages.create = AsyncMock(return_value=mock_response)
        result = await anthropic_fn("Hello!", max_tokens=1000, consumer_api_key="ck_test")
