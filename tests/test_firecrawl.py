...
async def test_firecrawl_insufficient_balance(firecrawl_fn, mock_m, with_ck):
    mock_m._billing.check_balance.side_effect = InsufficientBalanceError("no funds")

    with pytest.raises(InsufficientBalanceError):
        await firecrawl_fn("https://example.com")

    mock_m._billing.charge.assert_not_called()


async def test_firecrawl_gateway_error(firecrawl_fn, mock_m, with_ck):
