...
async def test_apify_happy_path(apify_fn, mock_m, with_ck):
    ...
    with patch("tools.apify_tool.ApifyClient") as mock_cls:
        ...
        result = await apify_fn("apify/web-scraper", {"startUrls": []})
...
async def test_apify_insufficient_balance(apify_fn, mock_m, with_ck):
    mock_m._billing.check_balance.side_effect = InsufficientBalanceError("no funds")

    with pytest.raises(InsufficientBalanceError):
        await apify_fn("apify/web-scraper", {})

    mock_m._billing.charge.assert_not_called()


async def test_apify_gateway_error(apify_fn, mock_m, with_ck):
...
async def test_apify_actor_failure_no_charge(apify_fn, mock_m, with_ck):
    with patch("tools.apify_tool.ApifyClient") as mock_cls:
        mock_cls.return_value.actor.return_value.call.side_effect = Exception("Actor failed")
        result = await apify_fn("apify/web-scraper", {})
