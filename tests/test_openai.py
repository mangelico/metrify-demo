...
async def test_openai_happy_path(openai_fn, mock_m, with_ck):
...
async def test_openai_insufficient_balance(openai_fn, mock_m, with_ck):
    mock_m._billing.check_balance.side_effect = InsufficientBalanceError("no funds")

    with pytest.raises(InsufficientBalanceError):
        await openai_fn("Hello!")

    mock_m._billing.charge.assert_not_called()


async def test_openai_gateway_error(openai_fn, mock_m, with_ck):
...
async def test_openai_api_failure_no_charge(openai_fn, mock_m, with_ck):
    with patch("openai.AsyncOpenAI") as mock_cls:
...
async def test_openai_prompt_too_long(openai_fn, mock_m, with_ck):
    long_prompt = "x" * 2001
    result = await openai_fn(long_prompt)
    assert result.startswith("Error:")
    assert "2000" in result
    mock_m._billing.charge.assert_not_called()


async def test_openai_max_tokens_capped(openai_fn, mock_m, with_ck):
    mock_message = MagicMock()
    ...
    with patch("openai.AsyncOpenAI") as mock_cls:
        mock_cls.return_value.chat.completions.create = AsyncMock(return_value=mock_response)
        result = await openai_fn("Hello!", max_tokens=1000)
